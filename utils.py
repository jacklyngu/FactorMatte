...
def tensor_flow_to_image(
    flow_uv, clip_flow=None, convert_to_bgr=False, global_max=None
):
    flow_np = flow_uv.permute(1, 2, 0).cpu().numpy()
    image = flow_to_image(flow_np, clip_flow, convert_to_bgr, global_max)
    image = torch.from_numpy(image).permute(2, 0, 1)
    return image.float() / 255.0 * 2 - 1
...
def flow_to_image(flow_uv, clip_flow=None, convert_to_bgr=False, global_max=None):
    """
    ...
    v = flow_uv[:, :, 1]
    rad = np.sqrt(np.square(u) + np.square(v))
    rad_max = global_max if global_max else np.max(rad)
    # import pdb
