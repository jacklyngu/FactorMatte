...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256):
    ...
    for label, im_data in visuals.items():
        if label not in os.listdir(image_dir):
...
def save_videos(webpage, visuals, width=256):
    ...
    for label, vid_data in sorted(visuals.items()):
        video_name = f"{label}.mp4"
        video_path = os.path.join(video_dir, video_name)
        frame_height, frame_width = vid_data.shape[-2:]
        video = cv2.VideoWriter(
            video_path, cv2.VideoWriter_fourcc(*"MP4V"), 25, (frame_width, frame_height)
        )
        for i in range(vid_data.shape[0]):
            ...
            # frame = util.add_title(frame, label)
        ...
        print(
            "You may see an OpenCV 'vp80 not supported' error message despite the video saving correctly. Please ignore it."
        )
