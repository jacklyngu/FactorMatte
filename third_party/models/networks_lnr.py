...
class MaskLoss_Noreduce(nn.Module):
    """Define the loss which encourages the predicted alpha matte to match the mask (trimap)."""

    def __init__(self):
        super(MaskLoss_Noreduce, self).__init__()
        self.loss = nn.L1Loss(reduction="none")

    def __call__(self, prediction, trimap, target):
        """Calculate loss given predicted alpha matte and trimap.
        ...
        Parameters:
            prediction (tensor) - - predicted alpha
            target (tensor) - - float, not like trimap only {0, -1, 1}
            trimap (tensor) - - only {0, -1, 1}, judged by if target > -1

        Returns: the computed loss for each training sample; size [bs, 1, 1, 1]
        """
        mask_err = self.loss(prediction, target)
        pos_mask = F.relu(trimap)
        neg_mask = F.relu(-trimap)
        pos_mask_loss = (pos_mask * mask_err).sum((1, 2, 3), keepdims=True) / (
            1 + pos_mask.sum((1, 2, 3), keepdims=True)
        )
        neg_mask_loss = (neg_mask * mask_err).sum((1, 2, 3), keepdims=True) / (
            1 + neg_mask.sum((1, 2, 3), keepdims=True)
        )
        loss = 0.5 * (pos_mask_loss + neg_mask_loss)
        return loss
...
class ConvBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if isinstance(self.conv, nn.modules.ConvTranspose2d):
            ...
            desired_width = width // self.s
            pady = 0.5 * (
                self.s * (desired_height - 1)
                + (self.k - 1) * (self.d - 1)
                + self.k
                - height
            )
            padx = 0.5 * (
                self.s * (desired_width - 1)
                + (self.k - 1) * (self.d - 1)
                + self.k
                - width
            )
        ...
        if x.shape[-2] != desired_height or x.shape[-1] != desired_width:
            ...
            x = x[
                :,
                :,
                int(np.floor(cropy / 2.0)) : -int(np.ceil(cropy / 2.0)),
                int(np.floor(cropx / 2.0)) : -int(np.ceil(cropx / 2.0)),
            ]
        if self.norm:
            x = self.norm(x)
        if self.activation:
...
class ResBlock(nn.Module):
    ...
    def __init__(
        self, channels, ksize=4, stride=1, dil=1, norm=None, activation="relu"
    ):
...
class kp2uv(nn.Module):
    ...
    def __init__(self, nf=64):
        ...
        self.id_pred = ConvBlock(
            nn.Conv2d, nf + 3, 25, ksize=3, stride=1, activation="none"
        )
        # head to predict UV coordinates for every body part class
        self.uv_pred = ConvBlock(
            nn.Conv2d, nf + 3, 2 * 24, ksize=3, stride=1, activation="tanh"
        )
...
class LayeredNeuralRenderer(nn.Module):
    ...
    def __init__(self, nf=64, texture_channels=16, texture_res=16, n_textures=25):
        ...
        self.texture = nn.Parameter(
            torch.randn(1, texture_channels, texture_res, n_textures * texture_res)
        )
        ...
        self.final_rgba = ConvBlock(
            nn.Conv2d, nf, 4, ksize=4, stride=1, activation="tanh"
        )
        ...
        self.upsample_block = nn.Sequential(
            ConvBlock(
                nn.Conv2d, upsampling_ic, nf, ksize=3, stride=1, norm=nn.InstanceNorm2d
            ),
    ...
    def forward(self, uv_map, id_layers, uv_map_upsampled=None, crop_params=None):
        ...
        for i in range(n_layers):
            ...
            sampled_texture = F.grid_sample(
                texture, uv_map_i, mode="bilinear", padding_mode="zeros"
            )
            ...
            if uv_map_upsampled is not None:
                ...
                sampled_texture_up = F.grid_sample(
                    texture, uv_map_up_i, mode="bilinear", padding_mode="zeros"
                )
                id_layers_up = F.interpolate(
                    id_layers[:, i : i + 1],
                    size=sampled_texture_up.shape[-2:],
                    mode="bilinear",
                )
                ...
                last_feat = F.interpolate(
                    last_feat, size=upsampled_size, mode="bilinear"
                )
                ...
                rgba_residual = self.upsample_block(
                    torch.cat((rgba, inputs_up, last_feat), 1)
                )
