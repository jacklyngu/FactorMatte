...
class BaseModel(ABC):
    ...
    def test(self, total_iters):
        ...
        if self.opt.gradient_debug:
            self.set_requires_grad(self.netOmnimatteGAN, True)
            self.netOmnimatteGAN.train()
            web_dir = os.path.join(
                self.opt.results_dir,
                self.opt.name,
                "{}_{}_{}".format(self.opt.phase, self.opt.epoch, self.opt.test_suffix),
            )
            self.forward()
            ...
            if isinstance(name, str):
                ...
                for key in list(
                    ...
                    if net.state_dict()[key].shape != state_dict[key].shape:
                        print(key, 'size mismatch! duplicating...')
                        repeat_times = [2]+[1]*(len(net.state_dict()[key].shape)-1)
                        state_dict[key] = state_dict[key].repeat(repeat_times)
