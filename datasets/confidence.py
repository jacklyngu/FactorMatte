...
if __name__ == "__main__":
    ...
    arguments.add_argument('--step', default=1, type=int)
    opt = arguments.parse_args()

    forward_flo = sorted(glob.glob(os.path.join(opt.dataroot, 'forward_flow_step'+str(opt.step), '*.flo')))
    backward_flo = sorted(glob.glob(os.path.join(opt.dataroot, 'backward_flow_step'+str(opt.step), '*.flo')))
    assert(len(forward_flo) == len(backward_flo))
    rgb_paths = sorted(glob.glob(os.path.join(opt.dataroot, 'rgb', '*')))
    print(f'generating {len(forward_flo)} confidence maps...from', '_flow_step'+str(opt.step))
    outdir = os.path.join(opt.dataroot, 'confidence_step'+str(opt.step))
